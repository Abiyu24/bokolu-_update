    ...
    def action_approve(self):
        ...
        for line in self.voucher_lines:
            ...
            move_lines.append((0, 0, {
                ...
                'origin': self.requester_id.name if self.requester_id else False,
            }))

        # Determine picking type
        if source_warehouse and dest_warehouse and source_warehouse.id != dest_warehouse.id:
            # Cross-warehouse transfer: Use outgoing picking type
            ...
            # For cross-warehouse, set destination to the destination warehouse's stock location
            location_dest_id = dest_warehouse.lot_stock_id.id if dest_warehouse else self.destination_location_id.id
        else:
            # Internal transfer
            picking_type = self.env['stock.picking.type'].search([
        ...
        if not location_dest_id:
            raise ValidationError(
                _('Destination location ID could not be determined. Please ensure the destination location is properly configured.'))
        ...
        # Link the transfer and update state
    ...
    #def action_approve(self):
     #   self.ensure_one()
       ...
       # for line in self.voucher_lines:
        ...
        #    available_quantity = self.env['stock.quant']._get_available_quantity(
           #     product, self.location_id)
            ...
            #if available_quantity < line.quantity:
             #   raise ValidationError(
              #      _('Insufficient quantity of product %s in source location. Available: %s, Requested: %s' % (
               #         product.display_name, available_quantity, line.quantity)))
      ...
      #  else:
       ...
       #     picking_type_id = self.env.ref('stock.picking_type_internal').id
        #3 #   picking_type = self.env['stock.picking.type'].search([
         #3 #      ('warehouse_id', '=', self.warehouse_id.id),
    ...
    def unlink(self):
        for rec in self:
            if rec.state != "draft":
                ...
                return super(StoreIssueVoucher, self).unlink()
